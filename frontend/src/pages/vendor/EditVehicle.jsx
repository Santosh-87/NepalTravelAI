...
const EditVehicle = () => {
    ...
    const loadVehicle = async () => {
        try {
            setLoading(true);
            const vehicles = await marketplaceService.getMyVehicles();
            ...
            // Populate features
            if (vehicle.features) {
                const featuresList = vehicle.features.split(',').map(f => f.trim());
                setFeatures(featuresList.length > 0 ? featuresList : ['']);
            }
    ...
    if (loading) {
        return (
            <VendorLayout>
                <div className="loading-state">Loading vehicle...</div>
            </VendorLayout>
        );
    }

    if (error && !formData.vehicle_name) {
        return (
            <VendorLayout>
